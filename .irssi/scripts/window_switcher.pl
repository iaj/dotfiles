	...
	unless ($scheduled) {
		...
		Irssi::timeout_add_once(100, sub {
			Irssi::statusbar_items_redraw ('window_switcher');
                        ...
                        if (1 && $prompt =~ /^(.)ws (.+)$/i && index($cmdchars,$1) != -1) {
                                my $arg = $2;
                                my $wins = find_wins($arg);

                                #print "arg: $arg, " . @$wins;

                                # XXX: if you use numbers you have to be fast! fix this, larger timeout? 300?
                                ...
                                if (scalar @$wins == 1) {
                                    ...
                                    Irssi::signal_add('send text', $x );

                                    Irssi::timeout_add_once(300, sub {
                                        Irssi::gui_input_set('');
                                        Irssi::signal_remove('send text', $x);
                                    }, []);

                                    #Irssi::timeout_add_once(300, sub {
                                    #    if (Irssi::parse_special('$L')) {
                                    #        Irssi::gui_input_set('');
                                    #    }
                                    #}, []);
