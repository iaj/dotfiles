...
#
# THANKS:
# - Thanks to to Stefan 'tommie' Tomanek's (stefan@pico.ruhr.de) crapbuster.pl
#   script (also GPLv2) for some code, see below remove_old_suggestions().
#
# BUGS:
...
# TODO: don't check spelling in URLs, check for http(s)://, ftp://
# TODO: also check after , ) etc.
...
# Removes old suggestions from the window. Thanks to Stefan 'tommie' Tomanek's
# (stefan@pico.ruhr.de) crapbuster.pl script (also GPLv2) for the code to
# remove certain messages from a irssi window; modifed.
sub remove_old_suggestions_from_window
{
    # Number of lines (from the bottom) to check/remove. A value greater than
    # 1 is necessary to catch the suggestions when there was some other text
    # printed between the calls to remove_old_suggestions_from_window().
    my $limit = 5;

    # Get lines of the current window and go to the last line.
    my $win = Irssi::active_win();
    my $view = $win->view;
    my $line = $view->get_lines;
    return unless defined $line; # abort if there are no lines, empty window
    $line = $line->next while defined $line->next;

    while (defined $line->prev) {
        # Abort if we already checked/removed $limit lines.
        last if $limit == 0;

        my $level = $line->{info}{level};
        my $text = $line->get_text(0); # 0 means no color

        my $copy = $line;
        $line = $line->prev;

        # Remove the line if it was one of our suggestions. \S+ is the time.
        if ($level == MSGLEVEL_CLIENTNOTICE &&
            $text =~ /^\S+ -\!- Irssi: Suggestions for/) {
...
sub spellcheck_key_pressed
{
    ...
    # remove old suggestions so we don't clutter the window
    remove_old_suggestions_from_window();
    ...
    # don't check URLs
    return if ($word =~ m{^(http|https|ftp)://});
    ...
    # make sure format codes are escaped
    $word =~ s/%/%%/g;
...
# Remove any old suggestions when we send the message.
sub message_send
{
    remove_old_suggestions_from_window();
}
...
Irssi::signal_add('send text', 'message_send');
